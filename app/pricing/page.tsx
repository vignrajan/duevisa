...
export const metadata: Metadata = {
  title: "Pricing | DueVisa",
  description: "Simple, transparent pricing for DueVisa — the immigration deadline tracker. Start free. Upgrade when you need more.",
};

const PLANS = [
  {
    name:"Free", price:"$0", period:"forever",
    desc:"Get started and protect your 3 most critical documents.",
    features:["3 documents","30 & 7 day reminders","Basic dashboard","Email support"],
    missing:["All 5 reminder intervals","Family members","SMS alerts","Document storage"],
    cta:"Start free", href:"/signup", highlight:false,
  },
  {
    name:"Pro", price:"$9", period:"/month", note:"Or $79/year — save $29",
    desc:"Full protection for you and your family. Everything included.",
    features:["Unlimited documents","All 5 reminder intervals (180/90/60/30/7 days)","5 family members","SMS reminders","Document storage","Priority support"],
    missing:[],
    cta:"Start Pro", href:"/signup?plan=pro", highlight:true,
  },
  {
    name:"Team", price:"$29", period:"/month",
    desc:"For companies managing employee immigration. HR-ready.",
    features:["Everything in Pro","Up to 20 employee profiles","HR admin dashboard","Bulk CSV import","Team renewal overview","Dedicated account support"],
    missing:[],
        ...
        <section style={{ background:"var(--bg-page-alt)", paddingTop:"3rem", paddingBottom:"5rem" }}>
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {PLANS.map(plan => (
                <div key={plan.name} className="relative flex flex-col rounded-2xl" style={{
                  padding:"2rem",
                  background: plan.highlight ? "var(--color-forest)" : "var(--bg-card)",
                  border: plan.highlight ? "2px solid var(--color-forest)" : "1px solid var(--border-default)",
                  boxShadow: plan.highlight ? "0 20px 60px rgba(10,92,74,0.25)" : "var(--shadow-card)",
                }}>
                  {plan.highlight && (
                  ...
                  <div className="mb-6">
                    <h2 className="font-bold text-xl mb-2" style={{ color: plan.highlight?"#eef5f0":"var(--text-primary)" }}>{plan.name}</h2>
                    <div className="flex items-baseline gap-1 mb-1">
                      <span className="font-bold text-4xl tracking-tight" style={{ color: plan.highlight?"var(--color-lime)":"var(--text-primary)" }}>{plan.price}</span>
                      <span className="text-sm" style={{ color: plan.highlight?"rgba(238,245,240,0.6)":"var(--text-muted)" }}>{plan.period}</span>
                    </div>
                    {"note" in plan && <p className="text-xs font-medium" style={{ color: plan.highlight?"rgba(200,245,98,0.85)":"var(--color-forest)" }}>{plan.note}</p>}
                    <p className="text-sm mt-2 leading-relaxed" style={{ color: plan.highlight?"rgba(238,245,240,0.7)":"var(--text-secondary)" }}>{plan.desc}</p>
                  </div>
                  <ul className="space-y-2.5 mb-8 flex-1">
                    {plan.features.map(f => (
                      <li key={f} className="flex items-start gap-2">
                        <CheckCircle2 size={15} className="flex-shrink-0 mt-0.5" style={{ color: plan.highlight?"var(--color-lime)":"var(--color-forest)" }} />
                        <span className="text-sm" style={{ color: plan.highlight?"rgba(238,245,240,0.85)":"var(--text-secondary)" }}>{f}</span>
                      </li>
                    ))}
                    {plan.missing.map(f => (
                      <li key={f} className="flex items-start gap-2">
                        <X size={15} className="flex-shrink-0 mt-0.5" style={{ color:"var(--text-muted)" }} />
                        <span className="text-sm" style={{ color:"var(--text-muted)" }}>{f}</span>
                      </li>
                    ))}
                  </ul>
                  <Link href={plan.href} className={`text-center py-3 px-5 rounded-xl font-bold text-sm cursor-pointer transition-all ${
                    plan.highlight ? "btn-primary-lime" : "btn-primary"
                  }`}>{plan.cta}</Link>
                </div>
              ))}
            </div>
